...
@Repository
public interface RapportScolaireRepository extends JpaRepository<Rapport_scolaire, Long> {
    List<Rapport_scolaire> findByEnfantId(Long enfantId);
    
    @Query("SELECT r FROM Rapport_scolaire r WHERE r.enfant.id = :enfantId AND r.annee_scolaire = :anneeScolaire")
    List<Rapport_scolaire> findByEnfantIdAndAnneeScolaire(@Param("enfantId") Long enfantId, @Param("anneeScolaire") String anneeScolaire);
