...
@Repository
public interface ParrainageDepenseRepository extends JpaRepository<ParrainageDepense, Long> {
    
    // Trouver toutes les dépenses prises en charge par un parrainage
    List<ParrainageDepense> findByParrainageId(Long parrainageId);
    ...
    List<ParrainageDepense> findByEnfantId(@Param("enfantId") Long enfantId);
    
    // Trouver toutes les dépenses prises en charge pour une dépense spécifique
    List<ParrainageDepense> findByDepenseId(Long depenseId);
