...
public class service_parrain {
    ...
    public List<dto_rapport_scolaire.Response> getRapportsScolairesEnfant(Integer parrainId, int enfantId, String anneeScolaire) {
    ...
    public dto_parrain.ResumeEnfantResponse getResumeCompletEnfant(Integer parrainId, int enfantId) {
        // Vérifier que le parrain parraine bien cet enfant
        Parrainage parrainage = em.createQuery(
            "select p from Parrainage p where p.parrain.id = :parrainId and p.enfant.id = :enfantId and p.statut = 'ACCEPTE'", 
            Parrainage.class)
